...
function openMenu() {
	return new Promise((resolve) => {
		...
		helpers.lockScroll(true, helpers.$header.find('.header__menu-mob'), 'header');

		helpers.$header.addClass('is-menu-opened');
		$('.header__menu-mob').removeClass('is-hidden');
		...
		setTimeout(() => {
			$('.header__menu-mob').addClass('is-active');
...
function closeMenu() {
	return new Promise((resolve) => {
		...
		helpers.lockScroll(false, helpers.$header.find('.header__menu-mob'), 'header');
		helpers.$body.css('padding-right', '');
		helpers.$header.css('right', '');

		helpers.$header.removeClass('is-menu-opened');

		$('.header__menu-mob').removeClass('is-active');

		setTimeout(() => {
			$('.header__menu-mob').addClass('is-hidden');
			$('.js-burger').removeClass('is-disabled').attr('disabled', false);
...
function init() {
	...
	helpers.$document
		.on('click.header', (e) => {
			let $container = $('.header__menu-mob');
		...
		.on('keyup.header', (e) => {
			if ((e.key === 'Escape' || e.key === 'Esc') && $('.header__menu-mob').hasClass('is-active')) {
				closeMenu();
				$('.js-burger').removeClass('is-active');
			}
		});
